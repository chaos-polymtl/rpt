...
X_train_x, X_test_x, Y_train_x, Y_test_x = train_test_split(X_scale[:,:6],dataset[:,6], test_size=0.2)
X_train_y, X_test_y, Y_train_y, Y_test_y = train_test_split(X_scale[:,:6],dataset[:,7], test_size=0.2)
X_train_z, X_test_z, Y_train_z, Y_test_z = train_test_split(X_scale[:,:6],dataset[:,8], test_size=0.2)
...
def create_model(neurons=1, layers=1, activation='elu', optimizer='adamax'):
    ...
    layer = 0
    while layer < layers:
        model.add(Dense(neurons, input_dim=6, kernel_initializer=keras.initializers.GlorotUniform(), activation=activation))
        layer += 1
