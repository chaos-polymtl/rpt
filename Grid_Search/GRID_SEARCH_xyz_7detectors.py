...
X_train_x, X_test_x, Y_train_x, Y_test_x = train_test_split(X_scale[:,:7],dataset[:,7], test_size=0.2)
X_train_y, X_test_y, Y_train_y, Y_test_y = train_test_split(X_scale[:,:7],dataset[:,8], test_size=0.2)
X_train_z, X_test_z, Y_train_z, Y_test_z = train_test_split(X_scale[:,:7],dataset[:,9], test_size=0.2)


# Grid search
def create_model(neurons=1, layers=1, activation='relu', optimizer='adam'):
    model = Sequential()
    layer = 0
    while layer < layers:
