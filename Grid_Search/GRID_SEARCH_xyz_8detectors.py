...
X_train_x, X_test_x, Y_train_x, Y_test_x = train_test_split(X_scale[:,:8],dataset[:,8], test_size=0.2)
X_train_y, X_test_y, Y_train_y, Y_test_y = train_test_split(X_scale[:,:8],dataset[:,9], test_size=0.2)
X_train_z, X_test_z, Y_train_z, Y_test_z = train_test_split(X_scale[:,:8],dataset[:,10], test_size=0.2)
...
def create_model(neurons=1, layers=1, activation='tanh', optimizer='adam'):
    ...
    layer = 0
    while layer < layers:
        model.add(Dense(neurons, input_dim=8, kernel_initializer=keras.initializers.GlorotUniform(), activation=activation))
        layer += 1
...
grid_result_z = grid.fit(X_train_z, Y_train_z)

# Summarize results z
means = grid_result_z.cv_results_['mean_test_score']
stds = grid_result_z.cv_results_['std_test_score']
params = grid_result_z.cv_results_['params']
for i in range(len(params)):
    params[i]['mean'] = means[i]
    params[i]['std'] = stds[i]
df = pd.DataFrame(params)
df.to_excel('grid_search_z.xlsx')
print("Best: %f using %s" % (grid_result_z.best_score_, grid_result_z.best_params_))
