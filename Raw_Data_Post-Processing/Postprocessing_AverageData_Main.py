import re
import pandas as pd
from Postprocessing1DataFile_Functions import *
import os
...
"""
######################## Interpolation #######################################
interpolation("shifted_counts_all.txt")
##############################################################################

######################## Average ##########################################################
n_pts=5 # Number of points used for the average
n_detector = len(amps_id_vector) # Number of detectors used during the experiment
average_datapoints("interpolaited_counts_all_datasetC.txt","x_y_robot_position.txt", n_pts, n_detector)
###########################################################################################
plt.show()
