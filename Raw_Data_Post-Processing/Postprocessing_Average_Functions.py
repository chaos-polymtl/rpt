...
def interpolation(file_name):
    ...
    for i in range(len(counts_vector)):
        ...
        with open("interpolaited_counts_all_datasetC.txt", "w") as file:
            for i in range(vector_length):
...
############################################################################## 
def average_datapoints(file_name_counts, file_name_pos, n_pts, n_detector):
    
    	# Average for the position data 
	df = []
	with open(file_name_pos) as csvfile:
    		reader = csv.reader(csvfile, quoting=csv.QUOTE_NONNUMERIC) # change contents to floats
    		for row in reader: # each row is a list
        		df.append(row)
	# Write the averaged position in a new file
	with open("averaged9_robot_position.txt", "w") as file:
		for i in range(round(len(df)/n_pts-0.5)): # To make sure the array is the right size
			row=np.zeros(6) # Number of position points taken at each time steps (3 positions, 3 angles)
			for j in range(len(row)): 
				for k in range(n_pts):
					row[j]+=df[i+k][j]/n_pts # Averaging for each of the 6 values over n_pts of time steps 	
				file.write(str(row[j]) + "	") # Writing each of the 6 values on the same line with tab between eachother
			file.write("\n")
	
	# Average for the counts data	
	counts_vector=read_file(file_name_counts) # read the counts file
	
	# Write the averaged counts in a new file 
	with open("averaged9_counts.txt", "w") as file:
		for i in range(round(len(counts_vector[0])/n_pts-0.5)): # To make sure the array is the right size
			row=np.zeros(n_detector) # The array "row" should have a length equal to the number of detectors 
			for j in range(len(row)): 
				for k in range(n_pts):
					row[j]+=float(counts_vector[j][i+k])/n_pts	
